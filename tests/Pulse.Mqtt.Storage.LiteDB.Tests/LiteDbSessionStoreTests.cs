...
public sealed class LiteDbSessionStoreTests
{
    [Fact]
    public async Task Subscriptions_round_trip_across_a_restart()
    {
        ...
        await using (var store = new LiteDbSessionStore(database.Path))
        {
            await store.SaveSubscriptionsAsync(filters, CancellationToken.None);
        }

        await using var reopened = new LiteDbSessionStore(database.Path);
