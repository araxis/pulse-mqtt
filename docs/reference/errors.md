...
| `MqttConnectRejectedException` | The broker answered CONNECT with a non-success reason; carries the `ReasonCode` | Inspect the reason: credentials, authorization, identifier |
| `TerminalMqttConnectException` | The reconnect machinery gave up: the decision classified a failure as final, or the attempt cap was reached | The client is `Faulted`; fix the cause, then `StartAsync` |
...
| `InvalidOperationException` | API misuse: typed call without a serializer, `StartAsync` while running, registration validation failures | Fix the code or configuration; messages name the exact problem |
